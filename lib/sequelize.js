...
Sequelize.prototype.authenticate = function(options) {
    return this.query('SELECT 1+1 AS result', Utils._.assign({
        raw: true,
        plain: true
    }, options)).return().catch(function(err) {
        throw new Error(err);
    });
};
...
Sequelize.prototype.insertCLOB = function(model, data, primaryKey, ClobFields, done) {
    ...
    if (ClobFields.length < 1) {
        return done({
            message: 'Clob fields is null'
        });
    }
    ...
    for (let i = 0; i < ClobFields.length; i++) {

        let key = ClobFields[i];

        ClobData[key] = data[key] || '';
        data[key] = this.fn("EMPTY_CLOB");
    ...
    model
        ...
        .then(function(result) {
            ...
            search[primaryKey] = result.dataValues[primaryKey];

            for (let i = 0; i < ClobFields.length; i++) {
                delete search[ClobFields[i]];
            }

            let lib = _this.connectionManager.lib;

            let table = [];
            if (model.options.schema)
                table.push(model.options.schema);

            let tbName = model.options.tableName || model.options.TABLE;
            table.push(tbName);
            table = table.join('.');
            ...
            // prepare set string, returning and params
            for (let key in ClobData) {
                set.push(key + '= EMPTY_CLOB() ');

                returning.keys.push(key);
                returning.params.push(':' + key);
            ...
            set = set.join(',');
            returning.keys = returning.keys.join(',');
            returning.params = returning.params.join(',');
            // end set string, returning and params

            // prepare where and params
            ...
            where = where.join(',');
            // end prepare where and params

            _this.connectionManager.getConnection(_this.config).then(function(connection) {

                connection.execute(
                    "UPDATE " + table + " SET " + set + " WHERE " + where + " RETURNING " + returning.keys + " INTO " + returning.params,
                    params, {
                        autoCommit: false
                    },
                    ...
                    function(err, result) {
                        ...
                        if (result.rowsAffected < 1) {
                            return done({
                                message: 'Error get a lob'
                            });
                        }

                        async.forEachOfLimit(ClobData, 5, function(value, key, nextKey) {
                            if (!value) {
                                return setImmediate(nextKey);
                            }

                            let lob = result.outBinds[key][0];
                            ...
                            lob.write(value, 'utf8', function() {
                                console.log("lob.write callback");
                            });
                            lob.end(null, 'utf8', function() {
                                console.log("lob.end callback");
                                setImmediate(() => nextKey());
                            });
...
Sequelize.prototype.updateCLOB = function(model, data, search, ClobFields,table_name, done) {
    ...
    model
        ...
        .then(function(result) {
            ...
            _this.connectionManager.getConnection(_this.config).then(function(connection) {
                console.log("UPDATE " + table_name + " SET " + set + " WHERE " + where + " RETURNING " + returning.keys + " INTO " + returning.params);
                
                connection.execute(
                    "UPDATE \"" + table_name + "\" SET " + set + " WHERE " + where + " RETURNING " + returning.keys + " INTO " + returning.params,
                    // 'UPDATE "users" SET "address"= EMPTY_CLOB()  WHERE "id" = :id RETURNING "address" INTO :address',
                    params, {
                        autoCommit: false
                    },
